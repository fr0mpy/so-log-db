...
test.describe('Shell to MFE Complete Flow', () => {
  test('start at Shell → click MFE link → navigate MFE pages', async ({ page }) => {
    ...
    // 2. Find and click link to MFE (Connectors)
    console.log('\n2. Click to MFE (Connectors)')

    // Look for link to /connectors
    const mfeLink = page.locator('a[href="/connectors"], a[href*="connectors"]').first()
    ...
    expect(mfeUrl).toContain('/connectors')
